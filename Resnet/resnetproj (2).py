...
dense121 = models.resnet18(pretrained=True)
num_ftrs = dense121.fc.in_features
dense121.fc = nn.Sequential(nn.Linear(512, 6),nn.Sigmoid())
print(dense121)

def count_params(model):
    return sum(p.numel() for p in model.parameters() if p.requires_grad) 
#set_parameter_requires_grad(dense121)
    ...
    for i in range(num_epochs):
        ...
        for p in phases:
            ...
            for data in dataloader[p]:
                ...
                preds = torch.where(output > .7, 1, 0) # old version: _, preds = torch.max(output, dim = 1)
            ...
            if p == 'validate':
                ...
                if scheduler:
                    scheduler.step()
                # Save interim model
        MOD_PATH = './models/resnet18_{}.pt'.format(i)
...
dataloader = {x: DataLoader(transformed_dataset[x], batch_size=bs,
                            shuffle=True, num_workers=4,pin_memory=True) for x in ['train', 'validate']}
...
MOD_PATH = './models/resnet18_full.pt'
